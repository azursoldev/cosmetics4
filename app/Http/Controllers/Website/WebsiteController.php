...
class WebsiteController extends Controller
{
    ...
    public function cart()
    {
        ...
        //Apply Initial Shipping - Default to Flat Rate
        if(count($shipping) == 0 && $cartTotal > 0){
            if(get_option('flat_rate_active') == 'Yes'){
            ...
            }elseif(get_option('free_shipping_active') == 'Yes' && $cartTotal >= get_option('free_shipping_minimum_amount')){ 
                // Fallback to Free Shipping if flat rate not available
                $shippingLabel = get_option('free_shipping_label');
                if(empty($shippingLabel)){
                ...
                Cart::condition($shipping);
            }elseif(get_option('local_pickup_active') == 'Yes'){
                // Fallback to Local Pickup
                $shippingLabel = get_option('local_pickup_label');
                if(empty($shippingLabel)){
