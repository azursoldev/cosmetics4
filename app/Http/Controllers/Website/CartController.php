...
class CartController extends Controller
{
    ...
    public function add_to_cart(Request $request, $product_id)
    {
        ...
        if($product->product_type != 'variable_product'){
            ...
            // Set Flat Rate as default shipping if no shipping method is set
            $shipping = Cart::getConditionsByType('shipping');
            if(count($shipping) == 0 && get_option('flat_rate_active') == 'Yes'){
                $shippingLabel = get_option('flat_rate_label');
                ...
                $shippingCondition = new \Darryldecode\Cart\CartCondition(array(
                    ...
                    'type'   => 'shipping',
                    'target' => 'total',
                    'value'  => '+'.get_option('flat_rate_cost', 0),
                    'order' => 1
                ));
    ...
    public function update_cart(Request $request){  
        ...
        if(Cart::getSubTotal() < get_option('free_shipping_minimum_amount')){
            Cart::removeCartCondition(get_option('free_shipping_label', 'Free Shipping'));
        }
            ...
            if(count($shipping) == 0){
                if($cartTotal >= get_option('free_shipping_minimum_amount')){ 
                    $shipping = new \Darryldecode\Cart\CartCondition(array(
                        'name'   => get_option('free_shipping_label', 'Free Shipping'),
                        'type'   => 'shipping',
                        'target' => 'total',
                        'value'  => '0',
                        'order' => 1
                    ...
                    $shipping = new \Darryldecode\Cart\CartCondition(array(
                        'name' => get_option('flat_rate_active') == 'Yes' ? get_option('flat_rate_label', 'Flat Rate') : get_option('local_pickup_label', 'Local Pickup'),
                        'type'   => 'shipping',
                        'target' => 'total',
                        'value'  => get_option('flat_rate_active') == 'Yes' ? '+'.get_option('flat_rate_cost', 0) : '+'.get_option('local_pickup_cost', 0),
                        'order' => 1
                    ));
                    Cart::condition($shipping); 
                }
    ...
    public function shipping_method(Request $request, $method){  
        ...
        if($method == 'free_shipping'){ 
            ...
            Cart::removeConditionsByType('shipping');
            $shipping = new \Darryldecode\Cart\CartCondition(array(
                'name' => get_option('flat_rate_label', 'Flat Rate'),
                'type'   => 'shipping',
                'target' => 'total',
                'value'  => '+'.get_option('flat_rate_cost', 0),
                'order' => 1
            ...
            Cart::removeConditionsByType('shipping');
            $shipping = new \Darryldecode\Cart\CartCondition(array(
                'name' => get_option('local_pickup_label', 'Local Pickup'),
                'type'   => 'shipping',
                'target' => 'total',
                'value'  => '+'.get_option('local_pickup_cost', 0),
                'order' => 1
            ));
            Cart::condition($shipping); 
        }  
